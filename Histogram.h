...
public:
    Histogram(){};
    virtual ~Histogram(){};
    virtual void allocate(size_t size) { data.assign(size, 0); }
    virtual void increment(size_t idx) { data[idx]++; }
    virtual size_t operator[](size_t idx) const { return data[idx]; }
