...
class ProgramOptions : public DocumentedOptionsGroup {
    ...
    public:
        ...
        TextSection help() const {
            ...
            section.header << header_;
            for(auto it : options_) {
                section.subsections.push_back(it->help());
            }
            section.footer << footer_;
            return section;
        }
...
class ProgramModesOptions : public DocumentedOptionsGroup {
    ...
    std::shared_ptr<ProgramOptions>& push_back(const std::string& mode_name, std::shared_ptr<ProgramOptions> val) {
        modes.emplace(mode_name, val);
        modes_order.push_back(modes.find(mode_name));
        return modes[mode_name];
    }
    std::shared_ptr<ProgramOptions>& at(const std::string& mode_name) {
    ...
    std::shared_ptr<ProgramOptions>& operator[](const std::string& mode_name) {
        auto pos = modes.find(mode_name);
        if(pos != modes.end()) {
            auto pos = modes.emplace(mode_name, std::make_shared<ProgramOptions>(mode_name));
            return pos.first->second;
        }
        return pos->second;
    }    
    std::shared_ptr<ProgramOptions>& defaultMode() {
        return modes[default_mode_];
    }
    ...
    bool parse(int argc, const char* argv[]) {
        ...
        if(argc > 2) {
            ...
            if(selected_mode != modes.end()) {
                ...
                selected_mode = modes.find(default_mode_);
    ...
    TextSection help() const {
        ...
        options.subsections.push_back(modes.find(default_mode_)->second->help());
        for(auto& mode : modes_order) {
            if(mode->first != default_mode_) {
                options.subsections.push_back(mode->second->help());
            }
        }
...
private:
    using modes_t = std::map<std::string, std::shared_ptr<ProgramOptions>>;
    modes_t modes;
    std::vector<modes_t::iterator> modes_order; //for printing purpose
    modes_t::iterator selected_mode;
