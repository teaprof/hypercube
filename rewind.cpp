...
public:
    ...
    void readHeaders() {
        ...
        f.seekg(header.nStates*sizeof(StateHeader) + sizeof(header), std::ios_base::end);
        f.read(reinterpret_cast<char*>(state_headers.data()), sizeof(StateHeader)*header.nStates);
    }
    ...
    RNG rewind(size_t offset) {
        ...
        for(size_t i = 0; i < state_headers.size(); i++) {
            if(state_headers[i].offset < offset) {
                if(!best_idx.has_value() || state_headers[i].offset > *state_headers[best_idx]) {
                    best_idx = i;
                }
            }
        }
