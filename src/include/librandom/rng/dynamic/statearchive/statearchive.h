...
class StateArchive {
    public:
        ...
        StateArchive(const std::string path, size_t state_size) : path_(path), state_size_(state_size), flock(path.c_str())
        {
            m.lock();
            flock.lock();
    ...
    private:
        ...
        size_t headerStartPos(size_t n_elements) {
            ...
            size_t pos = sizeof(n_elements32) + n_elements32*sizeof(HeaderElement);
            return pos; //from the end of file
        }

        void seekHeaderStartP() {
            assert(f.is_open());
            size_t pos = headerStartPos(header_elements_.size());
            f.seekp(pos, std::ios::end);
        }
        void seekHeaderStartG(size_t n_elements) {
            assert(f.is_open());
            size_t pos = headerStartPos(n_elements);
        ...
        void writeState(const std::vector<char>& state) { 
        ...
        void writeHeader(const std::vector<HeaderElement>& elements_) {
            ...
            f.write(reinterpret_cast<const char*>(&n_elements), sizeof(uint64_t));
        }
